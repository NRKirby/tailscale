...
func (b *LocalBackend) handleC2NUpdate(w http.ResponseWriter, r *http.Request) {
	...
	// * tailscale.exe copies itself and kicks off the update process msiexec stops
	// * this process during the update before the selfCopy exits(?) This doesn't
	//   return because the process is dead.
	//
...
// currently running cmd/tailcsaled. It's up to the caller to verify that the
